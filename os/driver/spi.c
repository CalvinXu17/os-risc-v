...
static void spi_set_tmod(uchar spi_num, uint32 tmod)
{
...
void spi_send_data_normal(spi_device_num_t spi_num, spi_chip_select_t chip_select, const uchar *tx_buff, uint64 tx_len)
{
    // configASSERT(spi_num < SPI_DEVICE_MAX && spi_num != 2);
    ...
    uchar v_misalign_flag = 0;
    uint32 v_send_data;
    if((uint64)tx_buff % frame_width)
        v_misalign_flag = 1;
...
{
    // configASSERT(spi_num < SPI_DEVICE_MAX && spi_num != 2);
    // uchar *v_buf = malloc(cmd_len + tx_len);
    uchar *v_buf = kmalloc(cmd_len + tx_len);
    uint64 i;
    for(i = 0; i < cmd_len; i++)
        v_buf[i] = cmd_buff[i];
    for(i = 0; i < tx_len; i++)
                               ...
                               uint64 cmd_len, uchar *rx_buff, uint64 rx_len)
{
    ...
    while(v_rx_len)
    {
        ...
        switch(frame_width)
        {
            ...
            default:
                for(index = 0; index < fifo_len; index++)
                    rx_buff[i++] = (uchar)spi_handle->dr[0];
                break;
        }
